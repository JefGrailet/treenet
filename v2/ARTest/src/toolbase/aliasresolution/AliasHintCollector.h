 ...
 * written inside the InetAddress object which corresponds to the probed IP. AliasHintCollector 
 * also associates a "probe token" to each IP, which is a number taken from a single counter 
 * (synchronized with a Mutex object), which is later used to locate the IP identifiers in time.
 *
 * This duo IP identifier/probe token is later used to resolve aliases and infer routers. The 
 * alias resolution itself is performed with the class AliasResolver.
 *
 * In this version adapted for the Alias Resolution assessment version, the code is slightly 
 * simplified due to the removal of reverse DNS (here, we only care about the IP ID part). Also, 
 * the interface has been modified to retrieve the IPs to probe from a Router object rather than 
 * using a C/C++ list (it simplifies the calling code).
 *
...
public:
    ...
    // Setter for the list of IPs to probe, using a Router object
    void setIPsToProbe(Router *toProbe);
