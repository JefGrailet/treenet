...
void SubnetSite::computeLabel()
{
    // If there is only one IP, cannot tell if it is a (contra-)pivot
    if(IPlist.size() == 1)
    {
        this->label = SubnetSite::INCOMPLETE_SUBNET;
        this->minTTL = IPlist.front()->TTL;
        this->maxTTL = IPlist.front()->TTL;
        return;
    }
    ...
    this->minTTL = shortestTTL;
    this->maxTTL = greatestTTL;
    ...
    else if((unsigned short) shortestTTL == (((unsigned short) greatestTTL) - 1))
    {
        ...
        for(list<SubnetSiteNode*>::iterator i = IPlist.begin(); i != IPlist.end(); ++i)
        {
            ...
            if(cur != NULL && cur->TTL == shortestTTL)
            {
                ...
                {
                    this->label = SubnetSite::ODD_SUBNET;
                    return;
                }
            }
        }
        this->label = SubnetSite::ACCURATE_SUBNET;
        this->contrapivot = tempContrapivot;
    }
...
string SubnetSite::refinedToString()
{
    ...
    if(this->label == SubnetSite::ACCURATE_SUBNET || this->label == SubnetSite::ODD_SUBNET || 
       this->label == SubnetSite::INCOMPLETE_SUBNET)
    {
        ss << this->getInferredNetworkAddressString() << "\n";
        if(this->label == SubnetSite::ACCURATE_SUBNET)
            ss << "ACCURATE\n";
        else if(this->label == SubnetSite::ODD_SUBNET)
            ss << "ODD\n";
        else
            ss << "INCOMPLETE\n";
...
bool SubnetSite::isCredible()
{
    ...
    unsigned short baseTTL = (unsigned short) this->minTTL;
    unsigned short diffTTL = (unsigned short) this->maxTTL - baseTTL;
