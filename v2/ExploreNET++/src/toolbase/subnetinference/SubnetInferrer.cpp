...
SubnetInferrer::SubnetInferrer(ExploreNETEnvironment *e, 
                               unsigned short li, 
                               unsigned short ui, 
                               unsigned short ls, 
...
env(e)
{
    ...
    if(protocol == ExploreNETEnvironment::PROBING_PROTOCOL_UDP)
    {
    ...
    else if(protocol == ExploreNETEnvironment::PROBING_PROTOCOL_TCP)
    {
        int roundRobinSocketCount = DirectProber::DEFAULT_TCP_UDP_ROUND_ROBIN_SOCKET_COUNT;
        if(env->usingFixedFlowID())
            roundRobinSocketCount = 1;
