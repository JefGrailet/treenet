...
void OutputHandler::usage(string programName)
{
    ...
    (*out) << "\t-?" << "\t" << "--help" << "\t\t\t\t" << "help" << endl;
    (*out) << "ExploreNET++, written by Jean-Francois Grailet (November 2015)" << endl;
...
void OutputHandler::printHeaderLines()
{
    ...
    (*out) << setw(17) << left << "Target IP" << setw(17) << left << "Subnet label" 
    << setw(5) << left << "SPC" << setw(5) << left << "SIC"
    << setw(20) << left << "Network Number" << setw(6) << "Size" 
...
void OutputHandler::printDiscoveredSite(SubnetSite *site)
{
    ...
    unsigned short siteLabel = site->getLabel();
    switch(siteLabel)
    {
    ...
    (*out) << setw(17) << left << targetIPstr << setw(17) << left << subnetStatusStr 
    << setw(5) << left << SPCstr << setw(5) << left << SICstr 
    << setw(20) << left << NNstr << setw(6) << left << sizeStr << " : " << data;
}
