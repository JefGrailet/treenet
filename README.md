...
All current versions are exclusively available for Linux and for the IPv4 protocol. There is currently no equivalent for IPv6.
...
Current focus is on the additionnal components of `TreeNET` v3. The whole suite will later introduce new algorithms to improve *neighborhood* inference and modeling (with bipartite graphs).
