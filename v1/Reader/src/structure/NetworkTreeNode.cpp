...
NetworkTreeNode::NetworkTreeNode(SubnetSite *subnet) throw (InvalidSubnetException)
{
    ...
    if (subnet->getStatus() == SubnetSite::SHADOW_SUBNET)
        this->labels.push_back(subnet->getPivotAddress());
    else
...
unsigned int NetworkTreeNode::getDegree()
{
    unsigned int ingressInterfaces = 1;
    
    if(parent != NULL && parent->getType() != NetworkTreeNode::T_ROOT)
    {
        ingressInterfaces = (unsigned int) parent->getLabels()->size();
    }
    
    unsigned int egressInterfaces = children.size();
    
    // egressInterfaces is decremented for each neighborhood children which is "on the way"
    list<NetworkTreeNode*> childrenS;
    list<NetworkTreeNode*> childrenN;
    for(list<NetworkTreeNode*>::iterator i = children.begin(); i != children.end(); ++i)
    {
    ...
    return ingressInterfaces + egressInterfaces;
}
...
void NetworkTreeNode::copyRouterInfoIntoRoute()
{
    ...
    InetAddress *route = this->getAssociatedSubnet()->getRoute();
    unsigned short index = this->getAssociatedSubnet()->getRouteSize() - 1;
