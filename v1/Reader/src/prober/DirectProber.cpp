...
const uint32_t DirectProber::DEFAULT_MAX_RECORD_ROUTE_ADDRESS_SIZE = 9;
...
verbose(v)
{
	...
	{
		throw SocketException("Unsopported protocol. Direct Prober only supports IPPROTO_ICMP, IPPROTO_UDP, and IPPROTO_TCP");
	}
	...
	{
		if(verbose)
			cout << "Send raw socket has been created, socket identifier is " << sendSocketRAW << endl;
	}
	...
	// Creates receive socket for ICMP (the protocol is always IPPROTO_ICMP, because we are collecting ICMP messages)
	...
	else
	{
		...
		{
			if(verbose)
				cout << "ICMP receive raw socket has been created, socket identifier is " << icmpReceiveSocketRAW << endl;
		}
	}
		      ...
		      (lowerBoundSrcPortICMPid + i + 1) < upperBoundSrcPortICMPid)
		{
			...
			while((lowerBoundSrcPortICMPid + i + 1)<upperBoundSrcPortICMPid)
			{
				...
				else
				{
					...
					if(verbose)
					{
						cout << "TCP or UDP receive raw socket has been created on port " << tcpudpReceivePorts[i] 
						<< ", socket identifier is " << tcpudpReceiveSockets[i] << endl;
					}
					break;
				}
				i++;
                                                           ...
                                                           int *numberOfSentPackets)
{
	...
	do
	{
		rec = singleProbe(src, dst, TTL, useFixedFlowID, false, 0);
                                                           ...
                                                           int *numberOfSentPackets)
{
	...
	do
	{
		rec = doubleProbe(src, dst, TTL, useFixedFlowID, false, 0);
		(*numberOfSentPackets) += rec->getProbingCost();
...
int DirectProber::GET_READY_SOCKET_DESCRIPTOR()
{
	if(FD_ISSET(icmpReceiveSocketRAW, &receiveSet) != 0)
	{
		if(verbose)
		{
			cout << "ICMP receive raw socket is ready for reading, socket identifier is " << icmpReceiveSocketRAW << endl;
		}
	...
	if(probingProtocol == IPPROTO_TCP)
	{
		if(FD_ISSET(tcpudpReceiveSockets[activeTCPUDPReceiveSocketIndex], &receiveSet) != 0)
		{
			if(verbose)
			{
				cout << "TCP receive raw socket is ready for reading on port " << 
				tcpudpReceivePorts[activeTCPUDPReceiveSocketIndex] << ", socket identifier is " << 
				tcpudpReceiveSockets[activeTCPUDPReceiveSocketIndex] << endl;
			}
