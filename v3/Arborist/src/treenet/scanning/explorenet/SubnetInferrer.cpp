                                              ...
                                              bool useLowerBorderAsWell)
{
    ...
    catch(SocketException &e)
    {
        ...
        /*
         * N.B.: delete on a NULL value is okay, because delete operator checks the value before 
                                     ...
                                     bool useLowerBorderAsWell)
{
    ...
    try
    {
        rec = probe(mate31, firstICMPEchoReply->getReqTTL());
    }
    catch(SocketException &se)
    {
        delete firstICMPEchoReply;
    ...
    if((*siteRecord) != NULL && !(*siteRecord)->isAnonymousRecord())
    {
        ...
        // Success! Probes siteRecord with TTL-1 and TTL-2
        else
        {
            success = true;
            if(verbose)
                                          ...
                                          bool useLowerBorderAsWell)
{
    ...
    catch(SocketException &se)
    {
        ...
        if(newRecorddTTLPlus1 != NULL)
        {
            delete newRecorddTTLPlus1;
            newRecorddTTLPlus1 = NULL;
        }
        
        if(newRecorddTTL != NULL)
        {
            delete newRecorddTTL;
            newRecorddTTL = NULL;
        }
        ...
        if(verbose)
        {
            logStream << "Interrupting subnet inference due to a failure while probing." << endl;
        }
                                         ...
                                         unsigned char subnetdTTL)
{
    ...
    ProbeRecord *newRecord = NULL;
    try
    {
        newRecord = probe(mate31, subnetdTTL + 1);
    }
    catch(SocketException &se)
    {
        throw;
                                         ...
                                         unsigned char subnetdTTL)
{
    ...
    if(!alias.isUnset()) // We have a valid alias
    {
        ...
        else
        {
            ...
            try
            {
                newRecord = probe(mate31, subnetdTTL);
            }
            catch(SocketException &se)
            {
                throw;
                                            ...
                                            unsigned char subnetdTTL)
{
    ...
    try
    {
        record = probe(mate31, subnetdTTL);
    }
    catch(SocketException &se)
    {
        throw;
