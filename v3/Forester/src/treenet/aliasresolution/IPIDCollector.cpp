...
void IPIDCollector::run()
{
    ...
    for(unsigned long int i = 0; i < nbThreads; i++)
    {
        ...
        catch(SocketException &se)
        {
            ...
            /*
             * N.B.: no need for StopException here, because there is only one salve of threads 
             * the parent class (AliasHintCollector) will see by itself if something is stopping 
             * by calling the method isStopping() from TreeNETEnvironment.
             */
        }
    ...
    // Waiting for all active threads to complete and getting token, IP-ID, time value tuples
