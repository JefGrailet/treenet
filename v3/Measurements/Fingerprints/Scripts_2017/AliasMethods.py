...
# Computes the ratio of application of each alias resolution method via stacked bar charts, just 
# like AliasProportions.py. The required command line is also similar; however, here, the 
# output file containing an ERS graph is required.
...
if __name__ == "__main__":
    ...
    for i in range(0, len(ASes)):
        ...
        while fileLines[j]:
            ...
            occurrences[5] = aliasList.count('Reverse DNS')
        ...
        ratioDNS.append((float(nbReverseDNS) / float(nbGoodAliasLists)) * 100)
    ...
    bottom4 = np.zeros((len(correctlyParsedASes), 1))
    for i in range(0, len(correctlyParsedASes)):
        bottom4[i] = bottom3[i] + ratioGroup[i]
    ...
    p5 = plt.bar(ind + padding, ratioDNS, width, color='#FFFFFF', bottom=bottom4)
    ...
    plt.legend((p1[0], p2[0], p3[0], p4[0], p5[0]), 
               ('Address-based', 'Ally', 'IP-ID Velocity', 'Group', 'DNS'), 
               bbox_to_anchor=(0.03, 1.02, 0.90, .102), 
               loc=3,
               ncol=5, 
               mode="expand", 
               borderaxespad=0.)

    plt.savefig("AliasMethods_" + yearOfMeasurements + "_" + dateOfMeasurements + ".pdf")
    plt.clf()
