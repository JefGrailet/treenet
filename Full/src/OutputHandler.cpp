...
void OutputHandler::usage(string programName)
{
    ...
    (*out) << "\t-?" << "\t" << "--help" << "\t\t\t\t" << "help" << endl;
    (*out) << "TreeNET v1.0, written by Jean-Francois Grailet (Academic year 2014 - 2015)" << endl;
...
void OutputHandler::printDiscoveredSite(SubnetSite *site)
{
    ...
    string targetIPstr = "*";
    TargetAddress target = site->getTargetAddress();
    if (!target.address.isUnset())
        targetIPstr = *(target.address.getHumanReadableRepresentation());
