...
SubnetInferrer::SubnetInferrer(InetAddress & lIPa,
...
{
	srand(time(0));
	prober = new DirectICMPProber(probeAttentionMessage, tout, prpp, li, ui, ls, us, false);
}
...
bool SubnetInferrer::growSubnet_Rule_3_B(
		const ProbeRecord *sitePrevRecord,
                                         ...
                                         unsigned char subnetdTTL)throw(SubnetBarrierException)
{
	...
	{
		/**
		 * exploreSubnet() method always first attempt to figure out whether the currentAddress is the alias. Also,
		 * it investigates the IP addresses in ascending order (from lower bound to upper bound).
		 * In case the currentAddress<currentAddress.mate31; we have not reached mate31 to investigate whether or not it is an alias.
		 * If mate31 really is an alias then having an ECHO_REPLY from it is totally OK. Now we have two options;
		 * 1- currentAddress<mate31: we simply return true (get current address added to the subnet), because
		 * next time mate31 will be investigated by exploreSubnet()  and if it is not the alias, it will raise
		 * SubnetBarrierPrefixException and cause this currentaddress as well as itself to be removed
		 * from the SiteSubnet on the other hand if it is alias everything is all right
		 * 2- currentAddress>mate31: then it means mate31 has been explored before and it is decided not to be an alias
		 * However, it could be part of the subnet or not. So again if we probe the mate31 with subnetdTTL we
		 * expect not to get an ICMP echo reply if we get than we raise SubnetBarrierException.
		 */
...
bool SubnetInferrer::discoverAlias_Rule_3_B(
		const ProbeRecord *sitePrevRecord,
		const InetAddress &currentAddress,
		unsigned char currentSubnetPrefix,
		unsigned char subnetdTTL) throw (SubnetBarrierException)
...
bool SubnetInferrer::discoverAlias_Rule_4_B(
		const ProbeRecord *sitePrevRecord,
	...
	if(subnetdTTL >= 2 && sitePrevPrevRecord != 0)
	{
		...
		else
		{
			if(newPrevPrevRecord->getRplyICMPtype() == DirectProber::ICMP_TYPE_TIME_EXCEEDED)
			{
				...
				{
				    /**
					 * If we get a time exceeded from a different ip address except the sitePrevPrevRecord
					 * or tracePrevPrevRecord either the load balancing is sending the packets on a third
					 * path or the probed ip is on the dTTL orbit but far from our site. We assume the second
					 * is more probable because we collected prevPrevRecords two times one populating site records
					 * and one while tracing to the destination.
					 */
...
{
	temporaryProbingCost = 0;
	unsigned char localSubnetPrefixLength = NetworkAddress::getLocalSubnetPrefixLengthByLocalAddress(this->localIPAddress);
	...
	for(InetAddress currentAddress=firstAddress;currentAddress<=lastAddress;currentAddress++)
	{
		if(currentAddress == this->localIPAddress)
...
{
	ProbeRecord *record = cache.fakeProbe(dst, TTL, this->useFixedFlowID);

	if(record == 0)
	{
		if(this->doubleProbe == true)
			record = prober->doubleProbe(localIPAddress, dst, TTL, this->useFixedFlowID, false, 0);
		else
			record = prober->singleProbe(localIPAddress, dst, TTL, this->useFixedFlowID, false, 0);
